...
function usersModule() {
  const rows = state.users.map((user) => {
    ...
    return `<article class="user-row user-card ${user.id === state.activeUserId ? 'active-user' : ''}"><div class="user-identity"><span class="user-avatar">${escapeHtml(shortName(user.name || '?'))}</span><div><strong>${escapeHtml(user.name)}</strong><span>Área ${escapeHtml(user.area || 'sin asignar')} · ${locations.length} ${locations.length === 1 ? 'ubicación' : 'ubicaciones'}${latest ? ` · Última: ${escapeHtml(latest.name)}` : ''}</span>${selector}<div class="location-chips">${chips}</div></div></div><div class="user-actions">${user.id === state.activeUserId ? '<span class="tag">Activo</span>' : ''}${activeLocation ? `<button class="btn secondary small" data-action="edit-location" data-user-id="${user.id}" data-location-id="${activeLocation.id}">${icon('pencil')} Editar ubicación</button>` : ''}<button class="btn secondary small" data-action="edit-user" data-id="${user.id}">${icon('pencil')} Gestionar</button><button class="btn secondary small" data-action="activate-user" data-id="${user.id}">${user.id === state.activeUserId ? 'Quitar' : 'Activar'}</button></div></article>`;
...
function openLocationReassignment(sourceUserId, sourceLocationId) {
  ...
  openModal('Eliminar ubicación y resolver bienes', `<p class="page-subtitle">${linked.length} bien${linked.length === 1 ? '' : 'es'} está${linked.length === 1 ? '' : 'n'} asignado${linked.length === 1 ? '' : 's'} a <strong>${escapeHtml(sourceLocation.name)}</strong>. Elige qué hacer con ellos antes de eliminar la ubicación.</p><div class="form-grid"><div class="field span-2"><label for="relocate-disposition">Al eliminar esta ubicación</label><select id="relocate-disposition" required><option value="" selected disabled>Selecciona una opción</option><option value="unassign">Dejar bienes sin asignar</option><option value="reassign" ${candidates.length ? '' : 'disabled'}>Reasignar bienes a otra ubicación</option></select></div><div class="field span-2" id="relocate-user-field" hidden><label for="relocate-user">Nuevo resguardante</label><select id="relocate-user">${userOptions || '<option value="">No hay otra ubicación disponible</option>'}</select></div><div class="field span-2" id="relocate-location-field" hidden><label for="relocate-location">Nueva ubicación</label><select id="relocate-location"></select></div></div>`, { footer: `<button class="btn secondary" data-action="edit-user" data-id="${sourceUserId}">Cancelar</button><button class="btn danger" disabled data-action="commit-location-reassignment" data-source-user-id="${sourceUserId}" data-source-location-id="${sourceLocationId}">Selecciona una opción</button>` });
...
function updateLocationReassignmentOptions() {
  ...
  const disposition = $('#relocate-disposition')?.value || ''; const reassign = disposition === 'reassign';
  userField.hidden = !reassign; locationField.hidden = !reassign;
  if (!disposition) { commit.disabled = true; commit.textContent = 'Selecciona una opción'; return; }
...
function updateUserDeletionDestinations() {
  ...
  if (!commit || !userField || !locationField) return;
  const disposition = $('#delete-user-disposition')?.value || ''; const reassign = disposition === 'reassign'; userField.hidden = !reassign; locationField.hidden = !reassign;
  if (!disposition) { commit.disabled = true; commit.textContent = 'Selecciona una opción'; return; }
  if (!reassign) { commit.disabled = false; commit.textContent = 'Eliminar y dejar sin asignar'; return; }
...
async function handleClick(event) {
  ...
  if (action === 'commit-location-reassignment') {
    ...
    const sourceUser = state.users.find((user) => user.id === sourceUserId);
    const sourceLocation = sourceUser?.locations?.find((location) => location.id === sourceLocationId);
    const disposition = $('#relocate-disposition')?.value; if (!['unassign', 'reassign'].includes(disposition)) { notify('Selecciona si deseas dejar los bienes sin asignar o reasignarlos.', 'warning'); return; } const reassign = disposition === 'reassign';
...
document.addEventListener('submit', async (event) => {
  ...
  if (event.target.id === 'profile-form') { event.preventDefault(); const form = new FormData(event.target); const [marca, modelo] = String(form.get('marcaModelo')).split('|').map((part) => part.trim()); try { new RegExp(String(form.get('regex'))); } catch { notify('La expresión regular no es válida.', 'error'); return; } await mutate(() => state.magicProfiles.push({ id: uuid(), regex: String(form.get('regex')).trim(), descripcion: String(form.get('descripcion')).trim(), marca, modelo, possession: String(form.get('possession')) }), 'Se agregó un perfil de autorrelleno.'); return; }
});
